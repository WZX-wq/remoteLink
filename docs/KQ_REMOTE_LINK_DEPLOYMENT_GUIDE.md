...
| iOS 被观看端屏幕共享 | ReplayKit 扩展已接入现有远控视频与应用声音发送链路；仅允许观看，不开放远程触控 | 需要签名包在 iPhone/iPad 真机验证直连/中继、前后台切换、停止广播和远端声音播放 |
...
当前 ReplayKit 扩展已把画面和应用声音接入现有远控传输链路。iOS 只作为被观看端，不支持远端注入触控：

- [ ] 分享页有“开始屏幕共享”入口。
- [ ] 能拉起 iOS 系统屏幕广播面板。
- [ ] 系统面板里能看到鲲穹远程桌面 Broadcast Extension。
- [ ] 开始广播后，App 内采集状态能显示视频帧数和分辨率。
- [ ] 另一台设备能通过直连和中继看到 iPhone/iPad 画面。
- [ ] 播放普通应用声音时，远端能听到同步声音；麦克风继续使用独立语音通话。
- [ ] 暂停、继续、停止广播和前后台切换后，画面与声音状态恢复正确。

说明：代码链路已经接通，但仍需签名包和两台真机完成最终验收。受保护或禁止录制的媒体内容可能不会被 ReplayKit 提供给扩展。
