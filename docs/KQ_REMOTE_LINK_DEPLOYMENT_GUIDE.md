...
| 支付宝支付 | 当前保留“提交表单拉起支付宝”的方案，点击立即支付后显示加载态再拉起 | 已安装支付宝可拉起；用户取消后回到 App 提示取消，不重复拉起 |
...
| iOS 支付宝支付 | 已做未安装支付宝时提示取消的逻辑要求 | 需要 TestFlight 真机验证：已安装/未安装支付宝两种状态 |
...
| 生产 hbbs 私钥自动填充 | 本地模板保留占位，未硬编码现用私钥 | 现用 `id_ed25519` 私钥只应从服务器导出，不能本地猜测或重新生成替代 |
...
```bash
export KQ_HBBS_PUBLIC_KEY="<生成的公钥>"
export KQ_HBBS_SECRET_KEY="<生成的私钥>"
...
如果需要单独创建 API 使用的数据库账号，可以参考脚本顶部注释里的 `CREATE USER` / `GRANT` 示例。执行后把账号信息填入 `.env`：

```bash
KQ_DB_HOST=<正式数据库地址>
KQ_DB_PORT=3306
KQ_DB_USER=<API 数据库账号>
KQ_DB_PASSWORD=<API 数据库密码>
...
```bash
KQ_DOWNLOAD_VERSION=<版本号>
KQ_DOWNLOAD_SHA256=<安装包 SHA256>
```
...
- 支付宝 App 是否已安装
- iOS/Android 是否走原生拉起路径，而不是二维码 fallback
