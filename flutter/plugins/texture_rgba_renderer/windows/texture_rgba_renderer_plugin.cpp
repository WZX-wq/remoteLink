...
void TextureRgbaRendererPlugin::HandleMethodCall(
  ...
  if (method_call.method_name().compare("createTexture") == 0) {
     ...
     if (add) {
      ...
      if (textures_.find(key) == textures_.end()) {
          ...
          // TextureRgba owns the registration and unregisters it in its
          // destructor. Erasing the owner here must not unregister it twice.
          textures_.erase(key);
  ...
  else if (method_call.method_name().compare("markFrameAvailable") == 0) {
      auto args = std::get<flutter::EncodableMap>(*method_call.arguments());
      auto key = std::get<int>(args.at(flutter::EncodableValue("key")));
      auto texture = textures_.find(key);
      result->Success(flutter::EncodableValue(
          texture != textures_.end() && texture->second->MarkFrameAvailable()));
  }
