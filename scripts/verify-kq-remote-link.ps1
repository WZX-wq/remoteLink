...
Invoke-Step "Server deployment template check" {
    ...
    foreach ($androidBridgeFile in $androidBridgeFiles) {
        if (-not (Test-Path $androidBridgeFile)) {
            throw "Android bridge file is missing: $androidBridgeFile"
        }
        $trackedBridgeFile = git ls-files -- $androidBridgeFile
        if (-not $trackedBridgeFile) {
            throw "Android bridge file is not tracked by Git: $androidBridgeFile"
        }
    }

    $rustBridge = Get-Content .\src\bridge_generated.rs -Raw -Encoding UTF8
    foreach ($needle in @('Generated by `flutter_rust_bridge`@ 1.80.1', 'IntoIntoDart<EventToUI>', '#[path = "bridge_generated.io.rs"]')) {
        if ($rustBridge -notmatch [regex]::Escape($needle)) {
            throw "Android Rust bridge is missing $needle"
        }
    }
    $dartBridge = Get-Content .\flutter\lib\generated_bridge.dart -Raw -Encoding UTF8
    foreach ($needle in @('Generated by `flutter_rust_bridge`@ 1.80.1', 'sealed class EventToUI', 'Stream<EventToUI> sessionStart')) {
        if ($dartBridge -notmatch [regex]::Escape($needle)) {
            throw "Android Dart bridge is missing $needle"
        }
    }
