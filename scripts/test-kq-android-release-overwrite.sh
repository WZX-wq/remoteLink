...
require_exactly_once "${build_script}" 'publish_android_apk_if_release "${MODE}" "${apk_path}" "${published_apk_path}"'
...
publish_android_apk_if_release release "${source_apk}" "${published_apk}" >/dev/null
...
publish_android_apk_if_release debug "${source_apk}" "${debug_apk}" >/dev/null
...
if publish_android_apk_if_release release "${fixture_dir}/does-not-exist.apk" "${missing_source_apk}" >/dev/null 2>&1; then
...
if publish_android_apk_if_release release "${source_apk}" "${copy_failure_apk}" >/dev/null 2>&1; then
...
PUBLISH_HELPER="${publish_helper}" SOURCE_APK="${source_apk}" DESTINATION_APK="${interrupted_apk}" \
  ...
  bash -c '
    ...
    publish_android_apk_if_release release "${SOURCE_APK}" "${DESTINATION_APK}"
  ' >/dev/null 2>&1 &
publisher_pid=$!
for _ in {1..100}; do
...
if ! PUBLISH_HELPER="${publish_helper}" SOURCE_APK="${source_apk}" DESTINATION_APK="${caller_trap_apk}" \
