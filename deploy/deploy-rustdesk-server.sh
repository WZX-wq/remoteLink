...
# Keep values explicitly supplied by the deployment workflow when an existing
# server-side .env is reused for its database credentials.
KQ_RUNTIME_OVERRIDE_NAMES=(
  KQ_RELAY_SERVER KQ_ENABLE_API KQ_API_PORT KQ_API_PUBLIC_PATH KQ_PUBLIC_API_URL
  KQ_ENABLE_LOCAL_DB KQ_SUBSITE_NAME KQ_API_WEB_BASE_URL
  ...
  KQ_APPLE_IAP_BUNDLE_ID KQ_APPLE_IAP_ISSUER_ID KQ_APPLE_IAP_KEY_ID
  KQ_APPLE_IAP_PRIVATE_KEY KQ_APPLE_IAP_PRIVATE_KEY_PATH KQ_APPLE_IAP_ENVIRONMENT
)
...
KQ_IOS_RELEASE_MODE="${KQ_IOS_RELEASE_MODE:-N}"
...
KQ_IOS_SERVICE_ENV_NAMES=(
  ...
  KQ_APPLE_IAP_KEY_ID
  KQ_APPLE_IAP_PRIVATE_KEY
  KQ_APPLE_IAP_PRIVATE_KEY_PATH
  KQ_APPLE_IAP_ENVIRONMENT
)
KQ_IOS_SERVICE_ENV_PATTERN='^(KQ_ACCOUNT_DELETION_MODE|KQ_IDENTITY_ACCOUNT_DELETE_URL|KQ_IOS_IAP_PRODUCTS|KQ_APPLE_IAP_BUNDLE_ID|KQ_APPLE_IAP_ISSUER_ID|KQ_APPLE_IAP_KEY_ID|KQ_APPLE_IAP_PRIVATE_KEY|KQ_APPLE_IAP_PRIVATE_KEY_PATH|KQ_APPLE_IAP_ENVIRONMENT)='
...
require_https_url() {
  ...
  local value="${!name:-}"
  if [[ ! "${value}" =~ ^https://[^/]+(/.*)?$ ]]; then
    echo "${name} must be a complete HTTPS URL for an iOS release deployment." >&2
    return 1
  fi
}

validate_ios_release_server_config() {
  if ! is_enabled "${KQ_IOS_RELEASE_MODE}"; then
    return 0
  fi

  if [[ "${COMPOSE_PROFILES}" != "api" ]]; then
    echo "An iOS release deployment requires the KQ API service." >&2
    return 1
  fi
  if [[ "${PUBLIC_HOST}" =~ ^[0-9.]+$ ]]; then
    echo "PUBLIC_HOST must be a DNS hostname, not a raw IP address, for an iOS release deployment." >&2
    return 1
  fi
  ...
  for name in KQ_IOS_IAP_PRODUCTS KQ_APPLE_IAP_BUNDLE_ID KQ_APPLE_IAP_ISSUER_ID KQ_APPLE_IAP_KEY_ID; do
    if [[ -z "${!name:-}" ]]; then
      echo "${name} is required for an iOS release deployment." >&2
      return 1
    fi
  done
  if ! command -v node >/dev/null 2>&1; then
    echo "node is required to validate KQ_IOS_IAP_PRODUCTS." >&2
    return 1
  fi
  if ! node --input-type=module - "${KQ_IOS_IAP_PRODUCTS}" <<'NODE'
...
NODE
  then
    echo "KQ_IOS_IAP_PRODUCTS must map every server package to a unique App Store product ID." >&2
    return 1
  fi

  if [[ -n "${KQ_APPLE_IAP_PRIVATE_KEY:-}" ]]; then
    return 0
  fi

  local container_key_path="${KQ_APPLE_IAP_PRIVATE_KEY_PATH:-}"
  if [[ ! "${container_key_path}" =~ ^/app/data/[^.][^[:space:]]*$ ]] || [[ "${container_key_path}" == *".."* ]]; then
    echo "KQ_APPLE_IAP_PRIVATE_KEY_PATH must be an existing /app/data/ path inside the API container." >&2
    return 1
  fi
  local host_key_path="${INSTALL_DIR}/data/${container_key_path#/app/data/}"
  if [[ ! -s "${host_key_path}" ]]; then
    echo "Apple IAP private key file is missing at ${host_key_path}." >&2
    return 1
  fi
}
...
verify_ios_release_public_api() {
  ...
  local probe_url="${KQ_PUBLIC_API_URL%/}/health"
  echo "Verifying iOS release HTTPS endpoint: ${probe_url}"
  if ! curl --fail --silent --show-error --connect-timeout 5 --max-time 20 \
      --resolve "${PUBLIC_HOST}:443:127.0.0.1" "${probe_url}" >/dev/null; then
    echo "The iOS release HTTPS endpoint is not reachable with a valid certificate on this server." >&2
    return 1
  fi
}
...
compose() {
  ...
  for name in \
    ...
    KQ_DOWNLOAD_MAX_REQUESTS_PER_WINDOW KQ_DOWNLOAD_RATE_WINDOW_MS \
    KQ_DOWNLOAD_MAX_PER_IP_CONCURRENT KQ_DOWNLOAD_MAX_GLOBAL_CONCURRENT KQ_APP_SCHEME \
    KQ_ADMIN_GATE_TOKEN KQ_REQUEST_GATE_STATE_FILE \
    KQ_DB_HOST KQ_DB_PORT KQ_DB_USER KQ_DB_PASSWORD KQ_DB_ROOT_PASSWORD KQ_DB_NAME KQ_DB_CREATE_DATABASE \
    ...
    KQ_ALIPAY_PUBLIC_KEY KQ_ALIPAY_PUBLIC_KEY_PATH KQ_ALIPAY_NOTIFY_URL KQ_ALIPAY_GATEWAY_URL \
    KQ_ACCOUNT_DELETION_MODE KQ_IDENTITY_ACCOUNT_DELETE_URL KQ_IOS_IAP_PRODUCTS \
    KQ_APPLE_IAP_BUNDLE_ID KQ_APPLE_IAP_ISSUER_ID KQ_APPLE_IAP_KEY_ID \
    KQ_APPLE_IAP_PRIVATE_KEY KQ_APPLE_IAP_PRIVATE_KEY_PATH KQ_APPLE_IAP_ENVIRONMENT; do
    if [[ -n "${!name:-}" ]]; then
      env_args+=("${name}=${!name}")
    fi
  done
...
print_optional_ios_service_env() {
  local name value
  for name in "${KQ_IOS_SERVICE_ENV_NAMES[@]}"; do
    if [[ -n "${!name:-}" ]]; then
      value="${!name}"
      if [[ "${name}" == "KQ_APPLE_IAP_PRIVATE_KEY" ]]; then
        value="${value//$'\r'/}"
        value="${value//$'\n'/\\n}"
      fi
      printf '%s=%s\n' "${name}" "${value}"
...
merge_optional_ios_service_env() {
  local env_file="$1"
  local has_ios_env="N"
  local name
  for name in "${KQ_IOS_SERVICE_ENV_NAMES[@]}"; do
  ...
  [[ "${has_ios_env}" == "Y" ]] || return 0

  local tmp_env
  tmp_env="$(mktemp)"
  if [[ -s "${env_file}" ]]; then
    "${SUDO[@]}" awk -v pat="${KQ_IOS_SERVICE_ENV_PATTERN}" '$0 !~ pat' "${env_file}" > "${tmp_env}" || true
  fi
  print_optional_ios_service_env >> "${tmp_env}"
  "${SUDO[@]}" cp "${tmp_env}" "${env_file}"
  "${SUDO[@]}" chmod 600 "${env_file}"
  rm -f "${tmp_env}"
}
...
merge_decrypted_optional_payment_env() {
  ...
  if source_file_has_env_pattern "${source_file}" "${KQ_IOS_SERVICE_ENV_PATTERN}"; then
    merge_env_file_entries_by_pattern "${env_file}" "${source_file}" "${KQ_IOS_SERVICE_ENV_PATTERN}"
    merged="Y"
  fi
  if [[ "${merged}" == "Y" ]]; then
...
decrypt_api_env_file() {
  ...
  if [[ "${plain_file_has_required_db_config}" == "Y" ]]; then
    ...
    echo "Installed decrypted KQ API env to ${env_file}."
      ...
      source_file_has_env_pattern "${plain_file}" "${KQ_IOS_SERVICE_ENV_PATTERN}"
    ); then
...
write_compose_env() {
  ...
  if [[ "${#missing[@]}" -gt 0 ]]; then
    if env_file_has_required_db_config "${env_file}"; then
      ...
      merge_optional_ios_service_env "${env_file}"
...
prepare_compose_runtime() {
  ...
  # A previous non-compose deployment may leave Restart=always units behind.
  # They race Docker Compose and can resurrect containers with stale .env values.
  local service
  for service in \
    kq-remote-link-watchdog.timer \
    kq-remote-link-watchdog.service \
    kq-remote-link-api.service \
    kq-remote-link-hbbs.service \
    kq-remote-link-hbbr.service; do
